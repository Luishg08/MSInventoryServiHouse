...
export class StorageController {
  ...
  @Get('withProducts')
  async getStoragesWithProducts(@Res() res: Response, @Req() req: Request) {
    // const { message, success } = this.generalService.verifyToken(req, 'getStoragesWithProducts');
    // if (!success) {
    //   return res.status(401).json({
    //     status: false,
    //     code: 401,
    ...
    try {
      const storages = await prisma.storage.findMany({
        where: {
          stock: {
            some: {} // Al menos un producto asociado
          }
        },
        include: {
          stock: true,
          ...
          location: {
            select: { address: true, department: true, city: true }
          }
        }
      });
      return res.status(200).json({
      ...
      return res.status(500).json({
        success: false,
        status: 500,
        message: 'Error fetching storages with products',
        error: error.message,
      });
    }
  }
